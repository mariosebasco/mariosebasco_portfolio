...
The controller consists of a PID loop. The loop controls the current of the motor. Since at stall the current is proportional to the torque the motor provides, robust current control is crucial in providing haptic feedback for mechanisms such as springs and walls which rely on force outputs. By controlling the force as a function of position, velocity, and acceleration (using an encoder) one is capable of making a user interact with virutal springs, dampers, masses, and many more possible combinations.
